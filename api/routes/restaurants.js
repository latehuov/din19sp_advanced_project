...
var restaurants = require('../models/restaurants_table');

router.get('/:id?', function (req, res, next) {
    if (req.params.id) {
        restaurants.getById(req.params.id, function (err, rows) {
            if (err) {
                res.json(err);
            } else {
                res.json(rows.rows);
        ...
        restaurants.get(function (err, rows) {
            if (err) {
                res.json(err);
            } else {
                res.json(rows.rows);
...
router.post('/', function (req, res, next) {
    restaurants.add(req.body, function (err, count) {
        if (err) {
            res.json(err);
        } else {
